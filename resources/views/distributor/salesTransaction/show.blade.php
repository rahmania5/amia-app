        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <div class="row">
                            ...
                            @if ($salesTransaction->status == "Menunggu dikirim" || $salesTransaction->status == "Selesai")
                                <a href="printFaktur" target="blank">
                    ...
                    {{ Form::open(['route' => ['distributor.sales_transaction.addItem'], 'method' => 'post']) }}
                    {{ csrf_field() }}
                    <div class="card-body">
                ...
                <div class="card">
                    ...
                    @endif
                    @if ($salesTransaction->status == "Belum dipesan")
                    @include('distributor.salesTransaction.addItem_form')                
                        <div class="col-10">
                            ...
                            <tbody>
                                @foreach($salesTransactionDetails as $std)
                                    <tr>
                                        ...
                                        @if ($salesTransaction->status == "Belum dipesan")
                                        <td>
                                            <form method="POST" action="{{ route('distributor.sales_transaction.removeItem', $std->id) }}">
                                            {{ csrf_field() }}
                                            {{ method_field('DELETE') }}
                    ...
                    <div class="card-footer">
                        @if ($salesTransaction->status == "Belum dipesan")
                        <form method="POST" action="{{ route('distributor.sales_transaction.order', $salesTransaction->id) }}">
                        {{ csrf_field() }}
                        ...
                        <div class="text-right">
                            <a href="{{ route('distributor.payment.create', $salesTransaction->id) }}">
                                <button type="button" class="btn btn-primary btn-round">Bayar Pesanan</button>
                            </a>    
                        </div>
                        @elseif ($salesTransaction->status == "Menunggu dikirim")
                        <form method="POST" action="{{ route('distributor.sales_transaction.finishTransaction', $salesTransaction->id) }}">
                        {{ csrf_field() }}
                        <div class="text-right">
                            <a href="{{ route('distributor.return.create', $salesTransaction->id) }}">
